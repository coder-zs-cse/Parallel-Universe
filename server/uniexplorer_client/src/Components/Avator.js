...
const Avatar = () => {
  ...
  const Avataaars = {
    ...
    getEditableTypes() {
      return Object.keys(this.paths).filter((p) => {
        return Object.keys(this.paths[p]).length > 1;
      });
    },
    _getChildOrDefault(obj, type, option) {
      if (typeof obj[type][option] != "undefined") {
        ...
        ["facialHair", "accessories"].includes(type) &&
        option == null
      ) {
        return () => "";
      } else {
    ...
    _createAvataaar(options) {
      ...
      let [topType, topTypeIsHat, topTypeZIndex] = this._getTopType(
        options.top
      );
      ...
      let clotheGraphicType = this._getShape(
        "clothingGraphic",
      ...
      let accessoriesColor = this._getColor(
        "palette",
      ...
      let content = `
        ...
        ${
          accessoriesType
            ? group(accessoriesType(accessoriesColor), 69, 85)
            : ""
        }
        ${2 === topTypeZIndex ? top : ""}
      `;
      if (options.style === "circle") {
        ...
        content = `
          ...
          <path d="M260 160c0 66.274-53.726 120-120 120S20 226.274 20 160 73.726 40 140 40s120 53.726 120 120z" fill="${
            (_a = options.background) !== null && _a !== void 0
              ? _a
              : this.colors.palette.blue01
          }"/>
    ...
    _getTopType(option) {
      switch (option) {
        /* Long Hair */
        case "bigHair":
          return [this.paths.top.bigHair, false, 0];
        case "bob":
          return [this.paths.top.bob, false, 0];
        case "bun":
          return [this.paths.top.bun, false, 1];
        case "curly":
          return [this.paths.top.curly, false, 0];
        case "curvy":
          return [this.paths.top.curvy, false, 0];
        case "dreads":
          return [this.paths.top.dreads, false, 0];
        case "frida":
          return [this.paths.top.frida, false, 0];
        case "fro":
          return [this.paths.top.fro, false, 0];
        case "froAndBand":
          return [this.paths.top.froAndBand, false, 0];
        case "miaWallace":
          return [this.paths.top.miaWallace, false, 0];
        case "longButNotTooLong":
          return [this.paths.top.longButNotTooLong, false, 0];
        case "shavedSides":
          return [this.paths.top.shavedSides, false, 0];
        case "straight01":
          return [this.paths.top.straight01, false, 0];
        case "straight02":
          return [this.paths.top.straight02, false, 0];
        case "straightAndStrand":
          return [this.paths.top.straightAndStrand, false, 0];

        /* Short Hair */
        case "dreads01":
          return [this.paths.top.dreads01, false, 1];
        case "dreads02":
          return [this.paths.top.dreads02, false, 1];
        case "frizzle":
          return [this.paths.top.frizzle, false, 1];
        case "shaggy":
          return [this.paths.top.shaggy, false, 2];
        case "shaggyMullet":
          return [this.paths.top.shaggyMullet, false, 0];
        case "shortCurly":
          return [this.paths.top.shortCurly, false, 1];
        case "shortFlat":
          return [this.paths.top.shortFlat, false, 1];
        case "shortRound":
          return [this.paths.top.shortRound, false, 1];
        case "shortWaved":
          return [this.paths.top.shortWaved, false, 1];
        case "sides":
          return [this.paths.top.sides, false, 1];
        case "theCaesar":
          return [this.paths.top.theCaesar, false, 1];
        case "theCaesarAndSidePart":
          return [this.paths.top.theCaesarAndSidePart, false, 1];

        /* Hats */
        case "hat":
          return [this.paths.top.hat, true, 0];
        case "winterHat01":
          return [this.paths.top.winterHat01, true, 2];
        case "winterHat02":
          return [this.paths.top.winterHat02, true, 2];
        case "winterHat03":
          return [this.paths.top.winterHat03, true, 2];
        case "winterHat04":
          return [this.paths.top.winterHat04, true, 2];

        /* Hijab */
        case "hijab":
          return [this.paths.top.hijab, true, 1];
        /* Turban */
        case "turban":
          return [this.paths.top.turban, true, 1];
        /* Eyepatch */
        case "eyepatch":
          return [this.paths.top.eyepatch, false, 1];
  ...
  //Avatar attributes [USER SELECTABLE]
  //_______________________________________________________
  const [skin, setskin] = useState("tanned");
  const [skins, setskins] = useState(["tanned", "yellow", "pale", "light"]);
  //_______________________________________________________
  const [topstr, settopstr] = useState(
    "dreads01, dreads02, frizzle, shaggyMullet, shaggy, shortCurly, shortFlat, shortRound, sides, shortWaved, theCaesar, bigHair, bob, bun, curly, curvy, dreads, frida, froAndBand, fro, miaWallace, shavedSides, straight01, straight02, eyepatch, turban, hijab, hat, winterHat01, winterHat02, winterHat03, winterHat04"
  );
  const [top, settop] = useState("shortWaved");
  const [tops, settops] = useState(topstr.split(", "));
  //_______________________________________________________
  const [hairstr, sethairstr] = useState(
  ...
  const [haircolors, sethaircolors] = useState(hairstr.split(", "));
  //_______________________________________________________
  const [clothingstr, setclothingstr] = useState(
  ...
  const [clothingcolor, setclothingcolor] = useState("shirtCrewNeck");
  const [clothingcolors, setclothingcolors] = useState(clothingstr.split(", "));
  //_______________________________________________________
  ...
  const AvatarCreator = (name, value) => {
